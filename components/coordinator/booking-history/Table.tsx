...
export default function App() {
  ...
  const handlePrint = () => {
    ...
    if (formElement) {
      ...
      const generateCanvas = () => {
        ...
        if (formElement) {
          ...
          html2canvas(formElement).then((canvas) => {
            ...
            printWindow?.document.write('<div style="font-family: Arial, sans-serif;">');
            if (printWindow) {
              printWindow.document.write('<html><head><title>Print Waybill</title><style>');

              // Add the styles for the print window
              ...
              printWindow.document.write('<h1>Waybill</h1>');

              const header = formElement.querySelector('.header');
              const waybillDetails = formElement.querySelector('.waybill-details');
              const orderDetails = formElement.querySelector('.order-details');
              const qrSection = formElement.querySelector('.qr-section');
              const addressSection = formElement.querySelector('.address-section');
              const additionalInfo = formElement.querySelector('.additional-info');
              const productInfo = formElement.querySelector('.product-info');
              const attempts = formElement.querySelector('.attempts');
              const footer = formElement.querySelector('.modal-footer');

              if (header) printWindow.document.write('<div class="header">' + header.innerHTML + '</div>');
              if (waybillDetails) printWindow.document.write('<div class="waybill-details">' + waybillDetails.innerHTML + '</div>');
              if (orderDetails) printWindow.document.write('<div class="order-details">' + orderDetails.innerHTML + '</div>');
              if (qrSection) printWindow.document.write('<div class="qr-section">' + qrSection.innerHTML + '</div>');
              if (addressSection) printWindow.document.write('<div class="address-section">' + addressSection.innerHTML + '</div>');
              if (additionalInfo) printWindow.document.write('<div class="additional-info">' + additionalInfo.innerHTML + '</div>');
              if (productInfo) printWindow.document.write('<div class="product-info">' + productInfo.innerHTML + '</div>');
              if (attempts) printWindow.document.write('<div class="attempts">' + attempts.innerHTML + '</div>');
              if (footer) printWindow.document.write('<div class="footer">' + footer.innerHTML + '</div>');
      ...
      // If all images are already loaded, generate the canvas
      if (loadedImages === images.length) {
        generateCanvas();
      }
    } else {
  ...
  const exportToPDF = () => {
    ...
    doc.setFontSize(18);
    const returnItemsTitle = "Booking History",
    ...
    filteredItems.forEach((item, index) => {
      ...
      const details = [
        ...
        `Backloads: ${item.backloads}`,
        `Total Payment: ${item.total_payment}`,
        `Order Status: ${item.order_status}`,
        `Booking Created By: ${item.booking_create_by}`,
        `Location Updated By: ${item.location_updated_by}`,
        `Location Updated At: ${item.location_updated_at}`,
        `Status: ${item.status}`
      ];
  ...
  return (
    <div className="bg-white  text-black rounded-lg shadow-sm p-10 overflow-x-auto">
<div className="flex justify-between items-center mb-4">
  ...
  <div className="flex justify-end space-x-1">
    ...
    <div className="relative">
      <Dropdown>
        <DropdownTrigger>
          <Button className="text-xs lg:text-base" color="primary">
            Select Columns
          </Button>
        </DropdownTrigger>
      ...
      <div className="mb-4 flex items-center space-x-4">
        <input
          ...
          className="px-4 py-2 border rounded-md w-full lg:w-1/2 bg-white"
        />
      </div>
